...
import os
import nidaqmx
from nidaqmx.constants import ForceUnits, BridgeConfiguration, ExcitationSource
...
time_data, id_data, coors_data, load_data= [],[],[],[]
...
load_path = os.path.join(folder,today+'_load_cell.csv')
...
with dai.Device(pipeline) as device, nidaqmx.Task() as task:
    # Initialize load cell
    task.ai_channels.add_ai_force_bridge_two_point_lin_chan('cDAQ1Mod1/ai3', "MyForceTask", min_val=-200,
                                                            bridge_config=BridgeConfiguration.FULL_BRIDGE,
                                                            voltage_excit_source=ExcitationSource.INTERNAL,
                                                            max_val=11000, units=ForceUnits.POUNDS, voltage_excit_val=5,
                                                            nominal_bridge_resistance=700, first_electrical_val=0,
                                                            second_electrical_val=2, first_physical_val=0,
                                                            second_physical_val=11000)
    task.timing.cfg_samp_clk_timing(10, sample_mode=nidaqmx.constants.AcquisitionType.CONTINUOUS)  # Hz
    found = True
    frameRgb = None
    # frameDisp = None
    ...
    while True:
        try:
            load_data.append([time.time(), task.read()])
        except:
            continue
        latestPacket = {}
        ...
        if cv2.waitKey(1) == ord('q'):
            ...
            break
if len(load_data) > 1:
    pd.DataFrame(load_data,columns=['time','load_lb']).to_csv(load_path,index=False)
if len(coors_data) > 1:
